...
export default function UserProfilePage() {
    const { id: userId } = useParams(); // from route /admin/users/:id
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                // load user info (in real app call /api/admin/users/:id)
                const users = await api.fetchUsers();
                const found = users.find((u) => u.id === userId) ?? null;
                if (mounted) setUser(found);

                // load tasks
                const t = await api.fetchUserTasks(userId);
    ...
    const handleApprove = async (taskId) => {
        ...
        try {
            ...
            const updatedTask = await api.approveTask(userId, taskId); // replace with axios.patch(`/api/admin/users/${userId}/tasks/${taskId}/approve`)
            setTasks((prev) => prev.map((t) => (t.id === taskId ? { ...t, status: updatedTask.status } : t)));
        } catch (err) {
            console.error("Approve failed", err);
            alert("Could not approve task. Try again.");
        } finally {
                                                    ...
                                                    <td className="border border-gray-300 p-3 text-center">
                                                        {task.status !== "approved" ? (
                                                            <button
                                                                ...
                                                                className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-black text-white text-sm"
